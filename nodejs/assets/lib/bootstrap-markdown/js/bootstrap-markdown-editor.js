...
! function ($) {
    ...
    Markdown.prototype = {
                ...
                for (y = 0; y < btnGroups.length; y++) {
                    ...
                    var z,
                        ...
                        for (z = 0; z < buttons.length; z++) {
                            ...
                            btnGroupContainer.append('<button type="button" class="' + btnClass + ' btn-default btn-sm" title="' + button.title + '" tabindex="' + tabIndex + '" data-provider="' + ns + '" data-handler="' + buttonHandler + '"' + buttonToggle + '><span class="' + buttonIcon + '"></span> ' + btnText + '</button>')
        ...
        createMDHeader: function(e, preWord){
            var preWordLength = preWord.length;
            ...
            if ((pointer = preWordLength + 1, content.substr(selected.start - pointer, pointer) == preWord + ' ') || (pointer = preWordLength, content.substr(selected.start - pointer, pointer) == preWord + '')) {
                e.setSelection(selected.start - pointer, selected.end)
                e.replaceSelection(chunk)
                cursor = selected.start - pointer
            } else if (selected.start > 0 && (prevChar = content.substr(selected.start - 1, 1), !! prevChar && prevChar != '\n')) {
                e.replaceSelection('\n\n' + preWord + ' ' + chunk)
                cursor = selected.start + preWordLength * 2
            } else {
                // Empty string before element
                e.replaceSelection(preWord + ' ' + chunk)
                cursor = selected.start + preWordLength + 1
            }
        ...
        [
            {
                ...
                [
                    ...
                    } ,
                    {
                        ...
                        title: '取り消し線',
                        icon: {
                            fa: 'fa fa-strikethrough' + fa_btn_size
                        },
                        callback: function(e){
                            // Give/remove ** surround the selection
                            var chunk, cursor, selected = e.getSelection();
...
//                        title: '下線',
//                        icon: {
//                            fa: 'fa fa-underline' + fa_btn_size
//                        },
//                        callback: function(e){
//                            alert('未実装');
//                        }
                ...
                [
                    ...
                    },
                    {
                        ...
                        callback: function(e){
                            ...
                            var code = "";
                            code = prompt('コード言語名を入力してください', 'javascript');
                            if (code != null) {
                                var tmpcode_header = '```' + code + '\n'; 
                                e.replaceSelection(tmpcode_header + chunk + '\n```\n');
                                cursor = selected.start + tmpcode_header.length;
                                // Set the cursor
                                e.setSelection(cursor, cursor + chunk.length);
                            }
                        }
                ...
                [
                    {
                        ...
                        icon: {
                            ...
                            fa: 'fa fa-picture-o' + fa_btn_size
                        },
                        callback: function(e){
                            ...
                            link = prompt('画像リンクを入力してください', 'http://');
                            if (link != null) {
                                // transform selection and set the cursor into chunked text
                                e.replaceSelection('![' + chunk + '](' + link + ' "画像タイトルをここに")');
                                cursor = selected.start + 2;
                                // Set the next tab
                                e.setNextTab('画像タイトルをここに');
                                // Set the cursor
                                e.setSelection(cursor, cursor + chunk.length);
                            }
                        }
