...
function tree(schema, options) {
    ...
    schema.pre('remove', function (next) {
        ...
        this.collection.remove({
            path: {
                '$regex': '^' + this.path + pathSeparator
            }
        }, next);
    });

    schema.method('getChildren', function (recursive, cb, childrenCondition) {
        ...
        if (childrenCondition !== undefined) {
            if (childrenCondition.status !== undefined)
                filter.status = childrenCondition.status;
        }
        //modified
        return this.model(this.constructor.modelName).find(filter).populate('fromuid').populate('touid').sort({
            upddate: 1
        }).exec(cb);
    });
